...
func (s *memoryStore) Set(id string, digits interface{}, timeout time.Duration) error {
	s.Lock()
	digitStr := fmt.Sprintf("%v", digits)
	s.digitsById[id] = []byte(digitStr)
...
func (s *memoryStore) Get(id string) (digits interface{}) {
...
func (s *memoryStore) IsExist(key string) bool {
	return false
}
