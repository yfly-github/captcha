...
var (
	flagLen = flag.Int("len", DefaultLen, "length of captcha")
)
...
func NewLen(length int) (id string) {
	id = randomId()
	globalStore.Set(id, RandomDigits(length), Expiration)
	return
}
...
func Reload(id string) bool {
	...
	globalStore.Set(id, RandomDigits(len(oldStr)), Expiration)
	return true
}
...
func WriteImage(w io.Writer, id string, width, height int) error {
	...
	_, err := NewImage(id, []byte(fmt.Sprintf("%v", d)), width, height).WriteTo(w)
	return err
}
...
func WriteAudio(w io.Writer, id string, lang string) error {
	...
	_, err := NewAudio(id, []byte(fmt.Sprintf("%v", d)), lang).WriteTo(w)
	return err
}
